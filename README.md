...
## Мобильное меню, массовое назначение, фото в рамке

- В кабинете тренера на телефоне меню стало drawer-панелью по кнопке «Меню», а не постоянным блоком сверху.
- Массовое назначение плана теперь сначала проверяет выбранных клиентов, сохраняет актуальный план, назначает его выбранным и очищает выбор.
- Фото на главной странице теперь заполняет всю карточку через `object-fit: cover`; инициалы/AC показываются только если фото не загружено.
